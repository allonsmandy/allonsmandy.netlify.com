...
Para entender melhor como podemos integrar o vuex em nossa aplicação Vue, vamos fazer um comparativo do gerenciamento de estado de ambos os casos!

![](assets/img/vue vuexbg.png)

A **Store** do Vuex é o ponto central onde será armazenado os dados da aplicação.

Nós sabemos que na instancia Vue nós criamos o **new Vue** e passamos um objeto de configuração, a mesma coisa vocẽ fará com o **Vuex Store**! Cria uma nova instancia pelo Vuex Store e passamos também um objeto de configurações.
...
Os componentes vão ***disparar***(dispatch) as ***ações***(actions), e as ações **commitam** as **mutations**, ou seja, confirmam essas ações e chama a mutation, a **mutation** vai receber os dados e *causar uma mudança no state* dai então meu componente será renderizado novamente.

O state pode ser acessado diretamente, ou então por meio dos **getters**, eu posso fazer um getter pra fazer uma filtragem, por exemplo.

As **mutations** precisam rodar de forma síncrona, já as **actions** não. Então como uma chama pro servidor é assíncrona, eu posso fazer dentro das **actions** essa chamada pra api, o servidor vai montar a resposta e quando tiver pronto ele devolve pra aplicação, e na action nós vamos ter um callback aguardando a resposta, no qual dentro dele extraímos os dados, daí quando a resposta chegar eu commito as mutation, a mutatiion pega a lista que ta chegando da action pra ela mesma e joga no state, o state sofre mudanças e então o componente é renderizado.
...
O primeiro item criado foi o **state** com a propriedade **user** que tem o valor ***Mandy***. Agora eu vou poder ter acesso a essa informação em TODOS os meus componentes do vue :)
...
Teremos acesso ao objeto **$store** em qualquer componente. Assim podemos acessar o estado de um item utilizando por exemplo **$store.state.user**.
...
Com a função **mapState** você consegue criar um atalho sem a necessidade de criar um por um, basta passar um array, e dar o nome de cada estado que deseja do **$store.state**.
...
Devemos sempre utilizar mutações para modificarmos um state(estado). Pense em mutations como se fossem métodos.
...
export default {
  ...
  methods: {
    handleClick() {
      ...
      this.$store.commit("changeUser", "Ave");
    }
  },
  computed: {
    ...mapState(["user"])
...
Também temos o mapMutations para facilitar o uso de mutações, podemos incluir as mesmas dentro dos métodos.
...
Quando precisamos de uma lógica ligada a alteração do state, ou caso exista a necessidade de dispararmos *mais de uma mutação*, utilizamos **actions**. As **actions** também permitem eventos assíncronos, diferente de mutations.
